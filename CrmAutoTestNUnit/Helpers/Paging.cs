...
namespace CrmAutoTestNUnit.Helpers
{
    public class Paging
    {
        ...
        public int ItemsFound(string url)
        {
        ...
        public int SelectPerPage(string url, IWebElement dropdownSelectPerPage)
        {
          
            int[] recOnPageArray = {20, 50, 100, 250};
            if (recOnPageArray[0] > ItemsFound(url))
            {
                PropertiesCollection._reportingTasks.Log(Status.Info, "ITEMS FOUND LESS : " + ItemsFound(url));
                return 0;
            }
            else
            {
                ...
                {
                    if (recOnPageArray[i] >= ItemsFound(url))
                    {
                        PropertiesCollection._reportingTasks.Log(Status.Info, "ITEMS FOUND OK : " + ItemsFound(url));
                        break;
                    }
                }
        ...
        /*public void CheckPaging(PagingData data)
        {
            ...
            //check logic
            int selectedValue = SelectPerPage(recordsFound, dropdownSelectPerPage);
            SeleniumGetMethod.WaitForPageLoad(PropertiesCollection.driver);
