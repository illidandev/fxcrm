...
namespace CrmAutoTestNUnit.Tests
{
    ...
    public class Settings:PropertiesCollection
    {
        public Settings(string browserName, string user) : base(browserName, user) { }
        ...
        public void CheckSortSettingsAllSubFolders()
        {
            var pageBase = _pages.GetPage<PageBase>();
            for (int i = 0; i < pageBase.SubFolderItemsSettings.Count; i++)
            {
                el.OpenTargetSubfolder(pageBase.LinksPanel[3], pageBase.SubFolderItemsSettings, i);
                pageBase.CheckSort();
            }
